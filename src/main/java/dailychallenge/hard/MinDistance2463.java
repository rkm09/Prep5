...
//        recursively calculate minimum total distance with memoization
        return calculateMinimumTotalDistance(0,0, robot, factoryPositions, memo);
    }

    private static long calculateMinimumTotalDistance(int robotIdx, int factoryIdx, List<Integer> robotPositions,
...
//        Option 1: assign the current robot to the current factory
        long assign = Math.abs(robotPositions.get(robotIdx) - factoryPositions.get(factoryIdx)) +
                    calculateMinimumTotalDistance(robotIdx + 1, factoryIdx + 1, robotPositions,
                            factoryPositions, memo);
//        Option 2: skip the current factory for the current robot
        long skip = calculateMinimumTotalDistance(robotIdx, factoryIdx + 1, robotPositions,
                    factoryPositions, memo);
//        take the minimum and store it in the memo
